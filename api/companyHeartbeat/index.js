...
module.exports = async function (context) {
  ...
  try {
    ...
    for (const c of campaigns) {
      ...
      // Include campaign brief so agents have context
      var _briefSnippet = c.description ? '\n\nCAMPAIGN BRIEF: ' + c.description.substring(0, 400) : '';
      _briefSnippet += '\n\nIMPORTANT: Use ONLY real data from your RECENT ACTIVITY and INTEL DIGEST sections. Do NOT invent numbers, metrics, or statistics. If you have no real data to cite, write about what AmbientOS does — not fabricated results.';

      const newTask = {
        id: 'task-' + Date.now() + '-' + Math.random().toString(36).substring(2, 6),
        title: 'Draft ' + label + ' for ' + (c.title || 'campaign'),
        description: 'Auto-created by campaign scheduler. Campaign: ' + (c.title || c.id) + '. Create a ' + label + ' aligned with the campaign brief and objectives.' + _briefSnippet,
        status: 'todo',
        taskType: chosenType,
        assignee: assignee,
        campaign_id: c.id,
