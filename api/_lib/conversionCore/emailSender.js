// emailSender.js — ConversionCore email delivery via Azure Communication Services

const ACS_CONNECTION_STRING = process.env.ACS_CONNECTION_STRING;

async function sendReportEmail(toEmail, report) {
  if (!ACS_CONNECTION_STRING) {
    console.warn('[CC Email] ACS_CONNECTION_STRING not configured, skipping email');
    return false;
  }
  ...
  try {
    ...
    const reportUrl = (process.env.CC_SITE_URL || 'https://ambientpixels.ai') +
      '/conversioncore/report.html?id=' + (report.id || '');
...
<body style="margin:0;padding:0;background:#071019;color:#e2e8f0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:640px;margin:0 auto;padding:40px 24px;">
    <div style="text-align:center;margin-bottom:32px;">
      <h1 style="color:#10b981;font-size:28px;margin:0;">ConversionCore</h1>
      <p style="color:#94a3b8;margin:8px 0 0;">Your Conversion Audit Report is Ready</p>
    </div>
    ...
    <div style="background:#0d1a2a;border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:32px;margin-bottom:24px;">
      <div style="text-align:center;margin-bottom:24px;">
        ...
        <p style="margin:12px 0 0;font-size:14px;color:#94a3b8;">Conversion Health Score</p>
    ...
    <div style="text-align:center;color:#64748b;font-size:12px;margin-top:40px;padding-top:24px;border-top:1px solid rgba(255,255,255,0.06);">
      <p>Powered by <a href="https://ambientpixels.ai" style="color:#10b981;text-decoration:none;">ConversionCore</a> by AmbientPixels</p>
    </div>
  </div>
</body>
</html>`;
    ...
    const message = {
      ...
      content: {
        subject: 'Your Conversion Audit: ' + (report.url || 'Website') + ' (Score: ' + report.score + '/100)',
        html: htmlBody
      },
      recipients: {
        to: [{ address: toEmail }]
    ...
    console.error('[CC Email] Send failed:', err.message);
    return false;
  }
}
