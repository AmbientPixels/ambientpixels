  ...
  try {
    ...
    if (!isAuthenticated) {
      context.res = {
        ...
        body: { error: 'Authentication required to view match history' }
      };
      return;
    }
    ...
    const history = await downloadJsonBlob(containerClient, historyPath, context);
    ...
    context.log.error(`[Arena History] Error: ${error.message}`);
    context.res = {
      status: 500,
      headers: CORS_HEADERS,
      body: { error: `Arena history error: ${error.message}` }
    };
  }
};
