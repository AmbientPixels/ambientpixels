...
module.exports = async function (context, req) {
    context.log('cardforge/mycards function processed a request');
    ...
    try {
        ...
        try {
            ...
            // Extract user's cards from the array
            // This API returns only the user's own cards, not published cards
            let userCards = [];
            if (Array.isArray(cardsData)) {
                userCards = cardsData.filter(card => {
                    return card && card.userId === userId && !card.isPublished;
                });
            }
        ...
        context.log.error(`Error retrieving my cards: ${error.message}`);
        context.res = {
            status: 500,
            headers,
            body: JSON.stringify({ error: `Failed to retrieve my cards: ${error.message}` })
        };
    }
};
