// as-confirm — POST /api/as-confirm
// Confirms an AmbientScore strategy session time slot.
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    if (token !== expectedToken) {
      context.log.warn('[as-confirm] Invalid token for', submissionId, 'slot:', slotIndex);
      context.res = {
        status: 403,
        headers: headers,
    ...
    if (!record) {
      context.log.warn('[as-confirm] Record not found:', submissionId);
      context.res = {
        status: 404,
        headers: headers,
    ...
    var dateEntry = {
      ...
      type: 'event',
      description: 'AmbientScore strategy call. Contact: ' +
    ...
    context.log('[as-confirm] Confirmed:', submissionId, 'slot:', slotIndex, confirmedSlot.label);
    ...
    context.log.error('[as-confirm] Error:', err.message, err.stack);
    context.res = {
      status: 500,
      headers: headers,
