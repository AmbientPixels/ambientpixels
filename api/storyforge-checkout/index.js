...
module.exports = async function (context, req) {
  ...
  try {
    ...
    // Only allow StoryForge products
    if (!productId.startsWith('sf-')) {
      context.res = {
        status: 400,
        headers: CORS_HEADERS,
        body: { error: 'Invalid product for StoryForge: ' + productId }
      };
      return;
    }
    ...
    const successUrl = SITE_URL + '/storyforge/?checkout=success&session_id={CHECKOUT_SESSION_ID}';
    const cancelUrl = SITE_URL + '/storyforge/?checkout=cancelled';
    ...
    context.log.error('[SF Checkout] Error: ' + error.message);
    context.res = {
      status: 500,
      headers: CORS_HEADERS,
