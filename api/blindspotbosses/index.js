...
// Load Blindspot boss data from static JSON file bundled with the function
let _bossCache = null;
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    let blindspotHighestDefeated = 0;

    // If authenticated, load their arena profile to check Blindspot PvE progress
    if (isAuthenticated) {
      try {
        ...
        if (profile && profile.pveProgress) {
          blindspotHighestDefeated = profile.pveProgress.blindspotHighestDefeated || 0;
        }
      } catch (err) {
        context.log.warn(`[Blindspot Bosses] Could not load profile for ${userId}: ${err.message}`);
      }
    }

    // Demo users see only the first Blindspot boss (bossLevel 101) unlocked
    ...
    const bosses = bossData.bosses.map(boss => ({
      ...
      stats: boss.stats,
      badges: boss.badges,
      locked: isDemo ? boss.bossLevel > 101 : boss.bossLevel > blindspotHighestDefeated + 1
    }));
    ...
    context.log.error(`[Blindspot Bosses] Error: ${error.message}`);
    context.res = {
      status: 500,
      headers: CORS_HEADERS,
      body: { error: `Blindspot bosses error: ${error.message}` }
    };
  }
};
